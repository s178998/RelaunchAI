...
export default function SkillsLearning() {
  ...
  useEffect(() => {
    ...
    const fetchSkills = async () => {
      try {
        ...
        console.log(
          "Fetching skills analysis for user:",
  ...
  // Get dynamic skill categories based on courses
  const getSkillCategories = () => {
    const allCourses = skillsData?.recommendedCourses || [];
    
    // Core Skills: matchScore >= 90
    const coreCourses = allCourses.filter(c => c.matchScore >= 90);
    
    // In Progress (Developing): matchScore 70-89
    const developingCourses = allCourses.filter(c => c.matchScore >= 70 && c.matchScore < 90);
    
    // Learning Goals (Aspiring): matchScore < 70
    const aspiringCourses = allCourses.filter(c => c.matchScore < 70);

    return [
      { id: "all", name: "All Skills", count: allCourses.length },
      { id: "core", name: "Core Skills", count: coreCourses.length },
      { id: "developing", name: "In Progress", count: developingCourses.length },
      { id: "aspiring", name: "Learning Goals", count: aspiringCourses.length },
    ];
  };
  ...
  const getFilteredCourses = () => {
    ...
    if (activeCategory === "core") {
      return allCourses.filter(c => c.matchScore >= 90);
    }
    if (activeCategory === "developing") {
      return allCourses.filter(c => c.matchScore >= 70 && c.matchScore < 90);
    }
    if (activeCategory === "aspiring") {
      return allCourses.filter(c => c.matchScore < 70);
    }
    return allCourses;
  };
  ...
  const getCourseUrl = (course) => {
    ...
    const providerUrls = {
      ...
      "Alex Xu (ByteByteGo)": `https://bytebytego.com/courses/system-design-interview`,
      "Martin Kleppmann": `https://dataintensive.net/`,
      "Full Stack Deep Learning": `https://fullstackdeeplearning.com/`,
      "DeepLearning.AI": `https://www.deeplearning.ai/courses/`,
      "NeetCode": `https://neetcode.io/`,
      "Zero To Mastery": `https://zerotomastery.io/courses/`,
      "MIT OpenCourseWare": `https://ocw.mit.edu/search/`,
      "Refactoring.guru": `https://refactoring.guru/`,
      "HashiCorp": `https://developer.hashicorp.com/terraform/tutorials`,
      "OWASP": `https://owasp.org/www-project-secure-coding-practices-quick-reference-guide/`,
      "Google": `https://cloud.google.com/learn/training`,
      "AWS": `https://aws.amazon.com/training/`,
    };

    // Return provider-specific URL or fallback to Google search
    return (
      providerUrls[course.provider] ||
      `https://www.google.com/search?q=${searchQuery}+course`
    );
  };
  ...
  const getRoleLearningMessage = () => {
    const role =
      JSON.parse(
        localStorage.getItem(`user_profile_${currentUser?.id}`) || "{}",
      )?.role || "professional";
      ...
      <div className="flex items-center justify-center min-h-[calc(100vh-120px)]">
        <div className="text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-2">
            Unable to load skills analysis
          </h3>
          ...
          <div className="bg-purple-50 rounded-xl p-5 border border-purple-100">
            ...
            <div className="flex flex-wrap gap-2">
              {skillsData.aspiring?.map((skill, idx) => (
                <span
                  key={idx}
                  className="px-2 py-1 bg-purple-100 text-purple-700 text-xs rounded-full cursor-pointer hover:bg-purple-200 transition"
                >
                  {skill}
                </span>
              ))}
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Personalized courses based on your skill gaps
              </p>
            </div>
              ...
              {filteredCourses.map((course) => (
                ...
                >
                  ...
                  <div className="w-full bg-gray-100 rounded-full h-1.5 mb-4">
                    <div
                      className={`h-1.5 rounded-full ${
                        course.matchScore >= 90 ? "bg-green-500"
                        : course.matchScore >= 70 ? "bg-yellow-500"
                        : "bg-blue-500"
                      }`}
                      style={{ width: `${course.matchScore || 85}%` }}
                    ></div>
                  </div>
                  ...
                  {selectedCourse === course.id && (
                    <div className="mt-4 pt-4 border-t border-gray-100">
                      <p className="text-xs text-gray-600">
                        This course is recommended based on your skill gap
                        analysis and current market demand for{" "}
                        {currentUser?.role || "your role"}.
                      </p>
                  ...
                  <button
                    onClick={() =>
                      setSelectedCourse(
                        course.id === selectedCourse ? null : course.id,
                      )
                    }
